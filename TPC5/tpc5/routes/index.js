...
router.get('/', function(req, res, next) {
  axios.get('http://localhost:7200/repositories')
       .then(async response =>{
              ...
              res.render('inicial',{repositories:dados})
        })
        .catch(erro => console.log('Error -> ' + erro))
});

router.get('/input/:id',(req,res) =>{
  res.render('getInput',{rep: req.params.id})
})
...
router.post('/input/:id', function(req, res, next){
  ...
  axios.get(endpoints[0] + rep + '?query=' + encoded)
       .then(response => {
          ...
          res.render('resultados',{length: head.length,head: head,resultados: response.data,query:query})
